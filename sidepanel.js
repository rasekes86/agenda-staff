// ============================================
// AGENDA STAFF v5.17.17 - MULTIPLE SIGNATURES
// ============================================
...
function setupPdfListeners() {
  ...
  // Setup split mode listeners for the new options
...
async function setSplitFile(file) {
  ...
  try {
    ...
    if (PDFLib) {
      ...
      splitPdfPageCount = pdfDoc.getPageCount();
    }
  } catch (err) {
  ...
  $('splitFileInfo').innerHTML = `
    <span class="pdf-icon">📄</span>
    <span>${file.name}</span>
    <span class="pdf-page-count">${splitPdfPageCount} páginas</span>
  `;
...
function clearSplitFile() {
  ...
  $('splitBlockSize').value = 2;
  // Reset radio to single mode
  ...
  try {
    ...
    if (splitMode === 'single') {
      ...
      for (let i = 0; i < pageCount; i++) {
        ranges.push({ start: i, end: i, label: `Página ${i + 1}` });
      }
    } else if (splitMode === 'blocks') {
      // Blocks of X pages
      const blockSize = parseInt($('splitBlockSize').value) || 1;
      for (let i = 0; i < pageCount; i += blockSize) {
        const start = i;
        const end = Math.min(i + blockSize - 1, pageCount - 1);
        ranges.push({ 
          start, 
          end, 
          label: `Páginas ${start + 1}-${end + 1}` 
        });
      }
      ...
      if (!rangesText) {
        throw new Error('Introduce los rangos personalizados');
      }
      ...
      for (const line of lines) {
        ...
        if (match) {
          ...
          if (start < 0 || end >= pageCount || start > end) {
            throw new Error(`Rango inválido: ${line}. El PDF tiene ${pageCount} páginas.`);
          }
          ...
          throw new Error(`Formato de rango inválido: ${line}. Usa el formato: inicio-fin`);
        }
      }
    }
    
    if (ranges.length === 0) {
      throw new Error('No hay rangos para procesar');
    }
    
    showToast(`Creando ${ranges.length} PDFs...`);
    
    // Generate PDFs for each range
    for (let i = 0; i < ranges.length; i++) {
      ...
      const fileName = `${baseName}_${range.label.replace(/ /g, '_')}.pdf`;
    ...
    showToast(`${ranges.length} PDFs creados correctamente`);
    closePdfModal();
    clearSplitFile();
...
// Update split preview based on current settings
function updateSplitPreview() {
  const preview = $('splitResultPreview');
  if (!preview || splitPdfPageCount === 0) return;
  
  const splitMode = document.querySelector('input[name="splitMode"]:checked')?.value || 'single';
  
  let html = '<div class="split-preview-title">Resultado:</div>';
  let count = 0;
  
  if (splitMode === 'single') {
    count = splitPdfPageCount;
    html += `<span class="split-preview-item">${count} PDFs (1 pág. cada uno)</span>`;
  } else if (splitMode === 'blocks') {
    const blockSize = parseInt($('splitBlockSize')?.value) || 2;
    count = Math.ceil(splitPdfPageCount / blockSize);
    html += `<span class="split-preview-item">${count} PDFs (${blockSize} págs. cada uno)</span>`;
    
    // Show block details
    const blocks = [];
    for (let i = 0; i < splitPdfPageCount; i += blockSize) {
      const start = i + 1;
      const end = Math.min(i + blockSize, splitPdfPageCount);
      blocks.push(`${start}-${end}`);
    }
    html += `<div style="margin-top:6px;font-size:10px;">Bloques: ${blocks.join(', ')}</div>`;
  } else if (splitMode === 'ranges') {
    const rangesText = $('splitRangesInput')?.value.trim() || '';
    if (rangesText) {
      const lines = rangesText.split('\n').filter(l => l.trim());
      ...
      html += `<span class="split-preview-item">${count} PDFs</span>`;
      
      // Validate and show ranges
      let valid = true;
      lines.forEach(line => {
        const match = line.match(/(\d+)\s*-\s*(\d+)/);
        if (!match) {
          ...
          if (start > end || start < 1 || end > splitPdfPageCount) {
            valid = false;
          }
        }
      });
      ...
      if (!valid) {
        html = '<div class="split-preview-title">Resultado:</div>';
        html += '<div class="split-preview-warning">⚠️ Algunos rangos son inválidos</div>';
      }
    } else {
      html += '<div class="split-preview-warning">Introduce los rangos</div>';
    }
  }
...
function setupSplitModeListeners() {
  ...
  // Listen for changes in block size
  $('splitBlockSize')?.addEventListener('input', updateSplitPreview);
  
  // Listen for changes in ranges input
...
async function addWordFiles(files) {
  for (const file of files) {
    ...
    if (ext !== 'docx') {
      showToast(`${file.name} no es un archivo DOCX válido`);
      continue;
    }
...
async function convertWordToPdf() {
  ...
  try {
    // Check if mammoth is available (it's a global variable)
    const mammothLib = window.mammoth || (typeof mammoth !== 'undefined' ? mammoth : null);
    if (!mammothLib) {
    ...
    if (typeof html2canvas === 'undefined') {
      throw new Error('La librería html2canvas no está cargada. Recarga la extensión.');
    }
    ...
    for (let i = 0; i < wordFiles.length; i++) {
      ...
      if (btnText) btnText.textContent = `Convirtiendo ${i + 1}/${totalFiles}...`;
      
      let renderContainer = null;
      
      try {
        // Convert DOCX to HTML using mammoth
        const arrayBuffer = wordFile.data;
        ...
        console.log('Mammoth conversion done for:', wordFile.name);
        ...
        renderContainer.id = 'word-render-container-' + Date.now();
        ...
        renderContainer.innerHTML = `
          <style>
            #${renderContainer.id} { box-sizing: border-box; }
            #${renderContainer.id} * { box-sizing: border-box; max-width: 100%; }
            #${renderContainer.id} h1 { font-size: 24pt; font-weight: bold; margin: 12pt 0 6pt 0; color: #000; }
            #${renderContainer.id} h2 { font-size: 18pt; font-weight: bold; margin: 10pt 0 4pt 0; color: #000; }
            #${renderContainer.id} h3 { font-size: 14pt; font-weight: bold; margin: 8pt 0 4pt 0; color: #000; }
            #${renderContainer.id} h1.title { font-size: 28pt; text-align: center; margin-bottom: 12pt; }
            #${renderContainer.id} p { margin: 6pt 0; text-align: justify; }
            #${renderContainer.id} strong, #${renderContainer.id} b { font-weight: bold; }
            #${renderContainer.id} em, #${renderContainer.id} i { font-style: italic; }
            #${renderContainer.id} u { text-decoration: underline; }
            #${renderContainer.id} s { text-decoration: line-through; }
            #${renderContainer.id} table { border-collapse: collapse; width: 100%; margin: 10pt 0; }
            #${renderContainer.id} table td, #${renderContainer.id} table th { 
              border: 1px solid #000; padding: 4pt 8pt; vertical-align: top; 
            }
            #${renderContainer.id} table th { font-weight: bold; background: #f0f0f0; }
            #${renderContainer.id} ul, #${renderContainer.id} ol { margin: 6pt 0; padding-left: 24pt; }
            #${renderContainer.id} li { margin: 2pt 0; }
            #${renderContainer.id} img { max-width: 100%; height: auto; display: block; margin: 10pt auto; }
            #${renderContainer.id} br { line-height: 1.6; }
          </style>
          ${html}
        `;
        ...
        const totalWidth = renderContainer.scrollWidth;
        
        console.log('Rendering dimensions:', totalWidth, 'x', totalHeight);
        ...
        while (currentY < canvas.height) {
          ...
          const sectionImg = tempCanvas.toDataURL('image/jpeg', 0.92);
    ...
    btn.disabled = false;
    if (btnText) btnText.textContent = 'Convertir a PDF →';
...
async function searchSignatures() {
  ...
  try {
    let allSignatures = [];
    let foundTerms = new Set(); // Track which terms were found
    
    // Search for each term
    for (const term of searchTerms) {
      const termLower = term.toLowerCase();
      // Query signatures table - search for exact or partial match
      const query = `?select=*&name=ilike.*${encodeURIComponent(termLower)}*&order=name.asc`;
      ...
      if (res.ok) {
        ...
        if (signatures && signatures.length > 0) {
          ...
          // Mark this term as found
          foundTerms.add(termLower);
        }
      }
    }

    // Determine which terms were NOT found (compare case-insensitive but preserve original case)
    const notFoundTerms = searchTerms.filter(term => {
      const termLower = term.toLowerCase();
      // Check if any signature name contains this term
      const hasMatch = allSignatures.some(sig => 
        sig.name.toLowerCase().includes(termLower) || 
        termLower.includes(sig.name.toLowerCase())
      );
      return !hasMatch && !foundTerms.has(termLower);
    });

    // Render results with both found and not found
    renderSignatureResultsWithMissing(allSignatures, notFoundTerms);
...
// Render signature results with both found and missing names
function renderSignatureResultsWithMissing(signatures, notFoundTerms) {
  let html = '';
  
  // Sort found signatures alphabetically
  signatures.sort((a, b) => a.name.localeCompare(b.name));
  
  // Summary header
  const foundCount = signatures.length;
  const missingCount = notFoundTerms.length;
  
  html += `<div class="signature-search-summary">`;
  if (foundCount > 0) {
    html += `<span class="signature-found-count">✓ ${foundCount} encontrada${foundCount !== 1 ? 's' : ''}</span>`;
  }
  if (missingCount > 0) {
    html += `<span class="signature-missing-count">✗ ${missingCount} no encontrada${missingCount !== 1 ? 's' : ''}</span>`;
  }
  html += `</div>`;
  
  // Found signatures section
  if (signatures.length > 0) {
    html += `<div class="signature-section-title">Encontradas</div>`;
    signatures.forEach(sig => {
      html += `
        <div class="signature-result-item found" data-id="${sig.id}" data-url="${sig.image_url}" data-name="${esc(sig.name)}" title="Clic para ${window.selectSignatureForEditor ? 'seleccionar' : 'descargar'}">
  ...
  // Missing signatures section (in red)
  if (notFoundTerms.length > 0) {
    html += `<div class="signature-section-title missing">No encontradas</div>`;
    notFoundTerms.forEach(term => {
      html += `
        <div class="signature-result-item missing" data-name="${esc(term)}" title="Clic para subir esta firma">
          <span class="signature-result-name">${esc(term)}</span>
          <button class="signature-upload-btn" data-name="${esc(term)}">
  ...
  document.querySelectorAll('.signature-result-item.found').forEach(item => {
    item.addEventListener('click', (e) => {
      // Don't trigger if clicking on missing item
      if (e.target.closest('.missing')) return;
  ...
  // Add click handlers for missing signatures - open upload section with name pre-filled
  document.querySelectorAll('.signature-upload-btn').forEach(btn => {
    btn.addEventListener('click', (e) => {
      e.stopPropagation();
      const name = btn.dataset.name;
      openSignatureUploadForName(name);
    });
  });
  
  // Also allow clicking the entire missing item row
  document.querySelectorAll('.signature-result-item.missing').forEach(item => {
...
// Open upload section with name pre-filled for a missing signature
function openSignatureUploadForName(name) {
  $('signaturesUploadSection').style.display = 'block';
  $('newSignatureName').value = name;
  $('signatureDropzone').style.display = 'block';
  $('signaturePreviewArea').style.display = 'none';
  $('signatureFileInput').value = '';
  signatureFile = null;
  // Scroll to upload section
  $('signaturesUploadSection').scrollIntoView({ behavior: 'smooth' });
}
...
async function uploadSignature() {
  ...
  // Save the current search to refresh results later
  const savedSearchTerm = signatureSearchTerm;
  ...
  try {
    ...
    // Reset upload section state (but keep modal open)
    signatureFile = null;
    if (signaturePreviewUrl && signaturePreviewUrl.startsWith('blob:')) {
      URL.revokeObjectURL(signaturePreviewUrl);
    }
    ...
    // Re-run the search to update results (shows newly uploaded signature as found)
    if (savedSearchTerm) {
      $('signatureSearchInput').value = savedSearchTerm;
